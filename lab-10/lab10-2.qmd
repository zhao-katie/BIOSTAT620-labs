...
We recommend downloading this QMD file to use as a template for your answers. You can download the file from: <https://github.com/dmcable/BIOSTAT620W26/blob/main/labs/lab10.qmd>
...
### FINISH THE CODE HERE
cv_states <- merge(cv_states, state_pops, "state")
```
...
```{r}
# format the date
cv_states$date <- ymd(cv_states$date)
...
cv_states_today_filter |> 
  plot_ly(x = ~pop_density, y = ~deathsper100k,
          type = "scatter", mode = "marker ", color = ~state,
          size = ~population, sizes = c(5, 70), 
          marker = list(sizemode="diameter", opacity = 0.5))
...
cv_states_today_filter |> 
  plot_ly(x = ~pop_density, y = ~deathsper100k,
          type = "scatter", mode = "marker", color = ~state,
          size = ~population, sizes = c(5, 70), 
          marker = list(sizemode = "diameter", opactiy = 0.5),
          hoverinfo = 'text',
          text = ~paste( 
            paste(state, ":", sep=""), 
            paste(" Cases per 100k: ", per100k, sep="") , 
            paste(" Deaths per 100k: ", deathsper100k, sep=""), sep = "<br>")) %>% 
  layout(title = "Population-normalized COVID-19\ndeaths (per 100k) vs. population density for US states",
...
-   For `pop_density` vs. `deathsper100k` create a chart with the same variables using `gglot_ly()`
...
plot_ly(cv_states, 
        x = ~date, y = ~naive_CFR, color = ~state,
...
cv_states |> filter(state=="Florida") |> 
  plot_ly(x = ~date, y = ~new_cases, type = "scatter", mode = "lines",
          name = "New Cases") |> 
  add_trace(x = ~date, y = ~new_deaths*naive_CFR*100, type = "scatter", mode = "lines",
            name = "New Deaths") 
# scale new_deaths by CFR to get on approximate scale
```

There appears to be a one-month delay between the peak of new cases and peak of new deaths. It makes sense that there would be a delay, but that a peak of new cases would proceed a peak of new deaths.
...
filter_dates <- seq(as.Date("2021-06-15"), as.Date("2021-11-01"), 
                    by= "2 weeks")
...
-   New Cases: There is a noticeable "spike" in new cases around January 2022, particularly in states such as California, Florida, Illinois, New York, and Texas

-   New Cases per 100,000: Now, the pattern of new cases spiking in January 2022 can be seen generally across all US states (as a band of the heatmap)

### 10. Map

-   Create a map to visualize the `newper100k` by state on October 15, 2021
-   Compare with a map visualizing the `newper100k` by state on most recent date
...
-   Describe the difference in the pattern of the newper100k.
...
fig <- fig |> layout(
    title = paste('Cases per 100k by State as of ', 
                  pick.date, '<br>(Hover for value)'),
    geo = set_map_details
  )
fig_pick.date <- fig
...
fig <- fig |> 
  colorbar(title = paste0("Cases per 100k: ", Sys.Date()), 
           limits = c(0,shadeLimit))
fig <- fig |> 
  layout(
    title = paste('Cases per 100k by State as of', 
                  Sys.Date(), '<br>(Hover for value)'),
    geo = set_map_details
  )
fig_Today <- fig
...
-   There appears to significantly fewer new cases per 100k in the "present day" compared to October 15th, 2021 (fewer dark purple shaded regions in the present day). On October 15th, 2021, the states with the highest cases per 100k metric were centralized to the Pacific Northwest region.
